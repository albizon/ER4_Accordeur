...
SercomSpi *spiFlash = (Sercom *)USES_SERCOM_PORT_EXTERNAL_FLASH_MEMORY;
#define SERCOM_FLASH_EXTERNAL_PM 0x0
#define SERCOM_FLASH_EXTERNAL_GCLK 0x0
...
void initSamd21Spi(void)
{
	...
	while ( spiFlash->CTRLA.bit.SWRST || spiFlash->SYNCBUSY.bit.SWRST );
...
void writeBytesSamd21Spi(uint8_t *bytes, uint32_t length)
{
	...
	for(uint32_t i=0; i<length-1; i++)
	{
		...
		while(!(spiFlash->INTFLAG.reg & 0x01));   //attente que Data Register Empty flag passe a 1
	}
